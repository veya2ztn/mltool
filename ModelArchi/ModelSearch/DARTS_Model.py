...
from .operations_define import MSOPS,MSOPS_NO,NORMALSIZE,REDUCESIZE
...
class MixedOp(nn.Module):
    ...
    def __init__(self, C, stride, input_dim,operation_choose=None,operation_candidate=None):
        ...
        for primitive in operation_choose:
            ...
            if 'pool' in primitive:op = nn.Sequential(op, nn.BatchNorm2d(C_in, affine=False))
            self._ops.append(op)
...
class Cell(nn.Module):
    ...
    def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev, input_dim,
                 operation_candidate,opertion_config=None,withbasic=True):
        ...
        if opertion_config is None:
            # initialize a default search config
            self.in_index        = [list(range(2 + node_order)) for node_order in range(self._nodes)]
            self.opertion_config = [[None for i in in_index_of_this_node] for in_index_of_this_node in self.in_index]
            # if #node==4, default operation [[None,None],[None,None,None],[None,None,None,None],[None,None,None,None,None]]
        else:
            self.in_index        = opertion_config[0]
            self.opertion_config = opertion_config[1]
        #print(self.reduction)
        for current_node,(in_nodes,edge_types) in enumerate(zip(self.in_index,self.opertion_config)):
            sub_map = []
        ...
        for edge_id,op in enumerate(self._ops):
            self.alphas_mask[edge_id,op._op_ids]=0
...
class Network(nn.Module):
    ...
    def __init__(self, C, num_classes, layers, input_dim=16, shrink_layer_index=None,
        ...
        self._map      = []
        self.arch_searchQ = opertion_config is None
        ...
        for i in range(layers):
            ...
            if i in self._shrink_layer_index:
                ...
                reduction = False
            opertion_config_for_this_cell = None if opertion_config is None else opertion_config[i]
            # cell_operation
            ...
            cell = Cell(steps, multiplier, C_prev_prev, C_prev,C_curr, reduction, reduction_prev, input_dim,
                        operation_candidate,opertion_config=opertion_config_for_this_cell,withbasic=withbasic)
    ...
    def genotype(self):
        ...
        for i, cell in enumerate(self.cells):
            ...
            for current_node,in_nodes in enumerate(cell.in_index):
                ...
                none_ops_idx = [_cd.index("none") for i,_cd in enumerate(all_ops_cd)]
                W[range(len(W)),none_ops_idx] = 0 #dont choose none operation
        ...
        def node_map(num):
            ...
            return num-2
        gene_str=""
        for cell_name,node in gene.items():
            gene_str+='\n'
            gene_str+=f'{cell_name}:'
            prefix   ="  -"
            for j,edge in node.items():
                for op_name,edge_idx in edge:
                    gene_str+='\n'+prefix+f'node_{node_map(edge_idx)} to node_{j}:{op_name}'
        return structure_config,gene_str
