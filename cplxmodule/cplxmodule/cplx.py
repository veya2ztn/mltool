...
def to_concatenated_real(input, flatten=None, dim=-1):
    """Concatenate the complex re-im pair into a real tensor."""
    assert flatten is None
    return torch.cat([input.real, input.imag], dim=dim)

def from_stacked_real(input, copy=True, dim=-1):
    """Map real tensor input `... x [D , 2 ]` to a pair (re, im) with dim `... x D`."""
    output = Cplx(input[...,0],input[...,1])
    return output.clone() if copy else output

def to_stacked_real(input, flatten=None, dim=-1):
    """Stacked the complex re-im pair into a real tensor."""
    return torch.stack([input.real, input.imag], dim=dim)
