...
class Fire(nn.Module):
    ...
    def __init__(self, inplanes, squeeze_planes,
        ...
        self.squeeze = nc.Conv2d(inplanes, squeeze_planes, kernel_size=1)
        self.squeeze_activation = nc.ReLU(inplace=True)
        self.expand1x1 = nc.Conv2d(squeeze_planes, expand1x1_planes,
                                   kernel_size=1)
        self.expand1x1_activation = nc.ReLU(inplace=True)
        self.expand3x3 = nc.Conv2d(squeeze_planes, expand3x3_planes,
    ...
    def __init__(self, version=1.0, num_classes=1000):
        ...
        if version == 1.0:
            self.features = nn.Sequential(
                nc.Conv2d(3, 96, kernel_size=7, stride=2),
                nc.ReLU(inplace=True),
                nc.AvgPool2d(kernel_size=3, stride=2, ceil_mode=True),
                Fire(96, 16, 64, 64),
                Fire(128, 16, 64, 64),
                Fire(128, 32, 128, 128),
                nc.AvgPool2d(kernel_size=3, stride=2, ceil_mode=True),
            ...
            self.features = nn.Sequential(
                nc.Conv2d(3, 64, kernel_size=3, stride=2),
                nc.ReLU(inplace=True),
                nc.AvgPool2d(kernel_size=3, stride=2, ceil_mode=True),
                Fire(64, 16, 64, 64),
                Fire(128, 16, 64, 64),
